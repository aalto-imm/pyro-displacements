...
from interface.CRYSTAL import CRYSTALParser
...
def check_code_options(args):
    conditions = [args.VASP is None,
                  ...
                  args.LAMMPS is None,
                  args.OpenMX is None,
                  args.CRYSTAL is None]

    if conditions.count(True) == len(conditions):
        raise RuntimeError(
            "Error : Either --VASP, --QE, --xTAPP, --LAMMPS, \
            --OpenMX, or --CRYSTAL option must be given.")

    elif len(conditions) - conditions.count(True) > 1:
        raise RuntimeError("Error : --VASP, --QE, --xTAPP, --LAMMPS, \
                            --OpenMX, and --CRYSTAL cannot be given simultaneously.")
        ...
        code = "CRYSTAL"
        struct_format = "CRYSTAL output file"
        str_outfiles = "%s{counter}.in %s{counter}.ext" % (args.prefix, args.prefix)
        file_original = args.CRYSTAL
...
def check_displace_options(args, code):
    conditions = [args.pattern_file is None,
                  ...
                  args.random_normalcoord_pyro is False,
                  args.pes is None]
    ...
    elif args.random_normalcoord_pyro:
        displacement_mode = "random_normalcoord_pyro"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--CRYSTAL',
                        metavar='crystal.o',
                        help="CRYSTAL output file with equilibrium atomic positions (default: None)")
    ...
    parser.add_argument('--random_normalcoord_pyro', action="store_true", dest="random_normalcoord_pyro", default=False,
                        help="Generate mode-specific displaced structures in normal coordinate basis. "
                             "Please give the --temp and --mode option as well.")
    ...
    parser.add_argument('--mode', type=float, default=False,
                        help="Target mode of the random distribution of \
                            Q in maximum mode-displacements.")
