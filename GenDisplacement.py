...
class AlamodeDisplace(object):
    ...
    def generate(self,
        ...
        if self._displacement_mode == "random_normalcoord_pyro":

            if self._verbosity > 0:
                print(" Displacement mode              : Mode-specific displacement in normal coordinate\n")
                print(" %d randomly-displaced configurations are generated from\n"
                      " the original supercell structure" % number_of_displacements)
                print(" The normal coordinate distribution is generated randomly from \n"
                      " the following conditions:\n\n"
            ...
            disp_random = self._get_random_displacements_pyro(number_of_displacements,
                                                                          temperature,
                                                                          ignore_imag, mode)
            for i in range(number_of_displacements):
                header = "Mode-specific displacement in normal coordinates at T = %f K: %i" % (temperature,
                                                                                 self._counter)
                header_list.append(header)
                disp_list.append(disp_random[:, :, i])
                self._counter += 1
    ...
    def _get_random_displacements_pyro(self, ndata, temperature, ignore_imag, mode):

        if temperature is None:
            raise RuntimeError("The temperature must be given with the --temp option.")

        #nq = len(self._qpoints) just one q point
        nq = 1
